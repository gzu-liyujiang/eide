                         ...
                         const char **grouping)
{
    ...
    static char* lc_decimal_point=".";
    static char* lc_thousands_sep="";
    static char* lc_grouping="";
    int change_locale = 0;
    if (decimal_point != NULL &&
        //(strlen(lc->decimal_point) > 1 || ((unsigned char)lc->decimal_point[0]) > 127))
        (strlen(lc_decimal_point) > 1 || ((unsigned char)lc_decimal_point[0]) > 127))
    {
        change_locale = 1;
    }
    if (thousands_sep != NULL &&
        //(strlen(lc->thousands_sep) > 1 || ((unsigned char)lc->thousands_sep[0]) > 127))
        (strlen(lc_thousands_sep) > 1 || ((unsigned char)lc_thousands_sep[0]) > 127))
    {
        change_locale = 1;
    }
    ...
    if (decimal_point != NULL) {
        *decimal_point = PyUnicode_DecodeLocale(lc_decimal_point, NULL);
        if (*decimal_point == NULL) {
            goto error;
        }
    }
    if (thousands_sep != NULL) {
        *thousands_sep = PyUnicode_DecodeLocale(lc_thousands_sep, NULL);
        if (*thousands_sep == NULL) {
            goto error;
        }
    }
