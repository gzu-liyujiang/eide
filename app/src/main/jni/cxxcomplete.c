...
static void
cxx_complete(JNIEnv *env, jobject self, jstring source, jint pos, jint replacePos,
             jstring source_file_path, jobjectArray cxflags) {
...
#undef INIT_CMD
    ...
    enum CXErrorCode EC =
            clang_parseTranslationUnit2(
                    ...
                    (const char *) cmds, n,
                    &sourceFile, 1,
                    clang_defaultEditingTranslationUnitOptions(), &translationUnit);
    LOGW("EC %d", EC);
        ...
        for (int i = 0; i < sizeof(DCC) / sizeof(DCC[0]); i++)
            if (__builtin_memcmp(input, DCC[i].text, len) == 0) {
                ...
                (*env)->CallVoidMethod(env, self, mid_add_complete,
                                       ...
                                       loc_hint = (DCC[i].hint == 0 ? 0 : (*env)->NewStringUTF(env,
                                                                                               DCC[i].hint)));
...
#undef BTH   */
    ...
    CXCodeCompleteResults *compResults = clang_codeCompleteAt(translationUnit, sourceFile.Filename,
                                                              line, column, &sourceFile, 1,
                                                              ccOpt);//clang_defaultCodeCompleteOptions());
  ...
  case CXCompletionChunk_##X:{\
    M;\
        ...
        for (unsigned j = 0; j < chunks; j++) {
            ...
            enum CXCompletionChunkKind cx_kind = clang_getCompletionChunkKind(
                    result.CompletionString, j);
